...
layout( location = 3 ) in vec3 in_normal_occ  ;    // normal del vértice 

//
// Valores calculados como salida ('out' aquí, 'in' en el fragment shader, distintos de cada vértice)
// se usa el prefijo v1_ para indicar que son varying de salida del Vertex Shaders con destino a los Tessellation shaders
//

out vec4 v1_posic_ecc ;   // posicion del punto (en coords de camara)
out vec4 v1_color ;       // color del vértice (interpolado a los pixels)
out vec3 v1_normal_ecc ;   // normal  (en coords. de cámara)
out vec2 v1_coord_text ;   // coordenadas de textura
out vec3 v1_vec_obs_ecc ; // vector hacia el observador (en coords de cámara)
