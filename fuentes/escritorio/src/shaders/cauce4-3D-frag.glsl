...
in vec4 v4_posic_ecc ;   // posicion del punto (en coords de camara)
in vec4 v4_color ;       // color del vértice (interpolado a los pixels)
in vec3 v4_normal_ecc;   // normal (en coords. de camara)
in vec2 v4_coord_text;   // coordenadas de textura
in vec3 v4_vec_obs_ecc ; // vector hacia el observador (en coords de cámara)
...
{
   return normalize( v4_vec_obs_ecc );
}
// -----------------------------------------------------------------------------------------------
...
{
   return normalize( cross( dFdx( v4_posic_ecc.xyz ), dFdy( v4_posic_ecc.xyz ) ) );   
}
// -----------------------------------------------------------------------------------------------
...
vec3 VectorHaciaFuenteEC( int i )
{
   if ( u_pos_dir_luz_ec[i].w == 1.0  )
      return normalize( u_pos_dir_luz_ec[i].xyz - v4_posic_ecc.xyz );
   else
...
void main()
{
   ...
   if ( u_eval_text  ) // si hay textura:
      color_obj = texture( u_tex, v4_coord_text );  // es el color de la textura en las coordenadas de textura actuales
   else  // si no hay textura:
      color_obj = v4_color ; // no hacer nada, simplemente usar color de entrada
